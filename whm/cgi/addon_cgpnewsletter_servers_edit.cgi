...
if ($FORM{'save'}) {
    ...
    Cpanel::CachedDataStore::store_ref( '/var/cpanel/cgpnewsletetr.yaml', $conf );
    $conf = Cpanel::CachedDataStore::fetch_ref( '/var/cpanel/cgpnewsletetr.yaml' ) || {};
}

my $settings = {%$conf};
if ($FORM{account}) {
    Whostmgr::HTMLInterface::defheader( "Server configuration for $FORM{account}",'', '/cgi/addon_cgpnewsletter_servers_edit.cgi' );
...
Cpanel::Template::process_template(
    'whostmgr',
    {
	'template_file' => 'addon_cgpnewsletter_servers_edit.tmpl',
	conf => $settings,
	FORM => \%FORM
	},
);
